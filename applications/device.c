...
void K0_Factory_Kick(void *parameter)
{
    LOG_D("Factory K0 is Kick\r\n");
    beep_kick();
}
void K1_Factory_Kick(void *parameter)
{
    LOG_D("Factory K1 is Kick\r\n");
    beep_kick();
}
...
{
    Key_Init();
    if(Read_K0_Level()==0 && Read_K1_Level()==0)return 1;
    else return 0;
}
void factory_button_init(void)
{
    Button_Create("Key0",&Key0,Read_K0_Level,0);
    Button_Create("Key1",&Key1,Read_K1_Level,0);
    Button_Attach(&Key0,BUTTON_DOWM,K0_Factory_Kick);
    Button_Attach(&Key1,BUTTON_DOWM,K1_Factory_Kick);
